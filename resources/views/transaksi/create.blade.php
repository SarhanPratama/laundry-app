                            ...
                            <div class="row g-3 layanan-item mb-3 align-items-end p-3 border rounded bg-light">
                                <div class="col-md-2">
                                    ...
                                    <select name="items[0][layanan_id]" class="form-control layanan-select" required>
                                        ...
                                        @foreach ($layananList as $l)
                                            <option value="{{ $l->id }}" data-harga="{{ $l->harga }}" data-satuan="{{ $l->satuan ?? 'Kg' }}">
                                                {{ $l->nama_layanan }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-1 text-nowrap">
                                    ...
                                    <input type="number" name="items[0][qty]"
                                        class="form-control form-control-sm qty-input" placeholder="0.0" step="0.1"
                                        min="0.1" required>
                                </div>
                                <div class="col-md-2">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Logic POS Visibility
        function togglePos() {
            let val = statusPembayaranSelect.value;
            // Check via jQuery if available (for nice-select compatibility)
            if (typeof jQuery !== 'undefined' && jQuery(statusPembayaranSelect).val()) {
                val = jQuery(statusPembayaranSelect).val();
            }

            if (val === 'Sudah Dibayar' || val === 'DP') {
                posContainer.style.display = 'block';
            } else {
                posContainer.style.display = 'none';
                bayarInput.value = 0; // Reset bayar
                calculateKembalian();
            }
        }

        statusPembayaranSelect.addEventListener('change', togglePos);

        // Add jQuery listener for nice-select
        if (typeof jQuery !== 'undefined') {
            jQuery(statusPembayaranSelect).on('change', togglePos);
        }
        ...
        addLayananBtn.addEventListener('click', function () {
            ...
            <div class="col-md-2">
                ...
                <select name="items[${newIndex}][layanan_id]" class="form-control layanan-select" required>
                    <option value="" data-harga="0">-- Pilih Layanan --</option>
                    @foreach ($layananList as $l)
                        <option value="{{ $l->id }}" data-harga="{{ $l->harga }}" data-satuan="{{ $l->satuan ?? 'Kg' }}">{{ $l->nama_layanan }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-2">
            ...
            <div class="col-md-1 text-nowrap">
                ...
                <input type="number" name="items[${newIndex}][qty]" class="form-control form-control-sm qty-input" placeholder="0.0" step="0.1" min="0.1" required>
            </div>
            <div class="col-md-2">
                <label class="form-label form-label-sm">Kategori Paket</label>
        ...
        function initLayananItemEvents(item) {
            ...
            const beratInput = item.querySelector('.qty-input');
            ...
            layananSelect.addEventListener('change', function () {
                ...
                const harga = selectedOption?.dataset.harga || 0;
                const satuan = selectedOption?.dataset.satuan || 'Kg';

                hargaLayananInput.value = harga;
                updateInputStyle(hargaLayananInput, 'auto');
                calculateSubtotal();

                const qtyLabel = item.querySelector('.qty-input').closest('div').querySelector('label');
                if (satuan === 'Kg') {
                    qtyLabel.innerHTML = `Berat (Kg) <span class="text-danger">*</span>`;
                    beratInput.setAttribute('step', '0.1');
                    beratInput.setAttribute('min', '0.1');
                    beratInput.setAttribute('placeholder', '0.0');
                } else {
                    qtyLabel.innerHTML = `Jumlah (Pcs) <span class="text-danger">*</span>`;
                    beratInput.setAttribute('step', '1');
                    beratInput.setAttribute('min', '1');
                    beratInput.setAttribute('placeholder', '0');
                    if (beratInput.value) beratInput.value = Math.ceil(beratInput.value);
                }
            });
            ...
            function calculateSubtotal() {
                ...
                const subtotal = (hargaLayanan + hargaKategori) * berat;
