...
<body>
    ...
    <div class="filter-info">
        ...
        @if ($request->filled('tanggal_dari') && $request->filled('tanggal_sampai'))
            Periode: {{ \Carbon\Carbon::parse($request->tanggal_dari)->format('d/m/Y') }} s/d
            {{ \Carbon\Carbon::parse($request->tanggal_sampai)->format('d/m/Y') }}<br>
            ...
            <tbody>
                @foreach ($data as $item)
                    <tr>
                        ...
                        <td class="text-right"><strong>Rp {{ number_format($item->total_harga, 0, ',', '.') }}</strong>
                        </td>
                                ...
                                <table class="detail-table" style="margin-top: 5px; border: none;">
                                    <thead>
                                        <tr style="background-color: #fff;">
                                            ...
                                            <th style="border: 1px solid #ddd; padding: 4px;">Jumlah/Berat</th>
                                            <th style="border: 1px solid #ddd; padding: 4px;">Harga Layanan</th>
                                        ...
                                        @foreach ($item->details as $detail)
                                            <tr style="background-color: #fff;">
                                                <td style="border: 1px solid #ddd; padding: 4px;">
                                                    {{ $detail->nama_layanan }}</td>
                                                <td style="border: 1px solid #ddd; padding: 4px;">
                                                    {{ $detail->nama_kategori ?? '-' }}</td>
                                                <td style="border: 1px solid #ddd; padding: 4px; text-align: center;">
                                                    {{ $detail->qty }}</td>
                                                <td style="border: 1px solid #ddd; padding: 4px; text-align: right;">Rp
                                                    {{ number_format($detail->harga_layanan, 0, ',', '.') }}</td>
